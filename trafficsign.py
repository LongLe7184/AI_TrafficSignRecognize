...
def main():
    ...
    plt.grid(True)
    plt.title('Do chinh xac cua mo hinh CNN')
    plt.xlabel('Epochs')
    plt.ylabel('Value')
...
def load_data(data_dir):
    ...
    for sub in range(number_of_labels):
        ...
        # Open each image 
            ...
            img_fromarray = Image.open(data_path + '/' + str(sub) + '/' + image).convert("RGB")
            resize_img = img_fromarray.resize((IMG_HEIGHT, IMG_WIDTH))
            images.append(np.array(resize_img))

            # print(image_path)
            print(data_path + '/' + str(sub) + '/' + image)
